...
class Status:
    ...
    def __init__(
        ...
        **kwargs,  # Accept extra kwargs for compatibility with curator CLI
        ...
        self.show_buckets = show_buckets or not any_section_specified
        self.show_ilm = show_ilm or not any_section_specified
    ...
    def _display_rich(
        ...
        # Settings table
        if self.show_config and self.settings:
            table = Table(title="Deepfreeze Configuration")
            table.add_column("Setting", style="white")
            table.add_column("Value", style="cyan")

            table.add_row("Provider", self.settings.provider)
            table.add_row("Repository Prefix", self.settings.repo_name_prefix)
            ...
            table.add_row("Rotation Style", self.settings.style)
            table.add_row("Last Suffix", self.settings.last_suffix)
            if self.settings.ilm_policy_name:
                table.add_row("ILM Policy", self.settings.ilm_policy_name)
            if self.settings.index_template_name:
                table.add_row("Index Template", self.settings.index_template_name)
