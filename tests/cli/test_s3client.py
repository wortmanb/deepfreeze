...
class TestS3ClientFactory:
    ...
    @patch("deepfreeze_core.gcp_client.storage")
    def test_factory_returns_gcp_client(self, mock_storage):
        """Test factory returns GcpStorageClient for 'gcp' provider"""
        from deepfreeze import s3_client_factory
        from deepfreeze_core import GcpStorageClient

        mock_gcp_client = MagicMock()
        mock_storage.Client.return_value = mock_gcp_client

        client = s3_client_factory("gcp")

        assert isinstance(client, GcpStorageClient)

    @patch("deepfreeze_core.azure_client.BlobServiceClient")
    def test_factory_returns_azure_client(self, mock_blob_service):
        """Test factory returns AzureBlobClient for 'azure' provider"""
        from deepfreeze import s3_client_factory
        from deepfreeze_core import AzureBlobClient

        mock_azure_client = MagicMock()
        mock_blob_service.from_connection_string.return_value = mock_azure_client

        client = s3_client_factory("azure", connection_string="test-connection-string")
